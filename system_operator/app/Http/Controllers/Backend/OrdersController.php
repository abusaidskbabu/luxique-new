...
class OrdersController extends Controller
{
    ...
    public function getOrderList($start_date, $end_date)
    {
        ...
        if (Auth::user()->getRoleNames() == '["seller"]') {
            $data = Order::select('orders.*', 'order_details.seller_id', 'order_details.is_promotion')->join('order_details', 'orders.id', '=', 'order_details.order_id')
                ->where('order_details.seller_id', Auth::user()->id)->with('statuses')->where('order_details.is_promotion', 0)->where('orders.is_deleted', 0)->orderBy('orders.id', 'desc');

            if ($start_date != 0 && $end_date != 0) {
                $data->whereBetween('orders.created_at', [$start_date, $end_date]);
            }
            $data->groupBy('orders.id')->with('statuses', 'user', 'address');
        } else {

            $data = Order::select('orders.*', 'order_details.seller_id', 'order_details.is_promotion')->join('order_details', 'orders.id', '=', 'order_details.order_id')->where('order_details.is_promotion', 0)->where('orders.is_deleted', 0);
            if ($start_date != 0 && $end_date != 0) {
                $data->whereBetween('orders.created_at', [$start_date, $end_date]);
            }
            $data->groupBy('orders.id')->with('statuses', 'user', 'address');
    ...
    public function soldInvoices(){
        return view('backend.pages.order.sold-invoice');
    }


    public function getSoldInvoices()
    {

        if (is_null($this->user) || !$this->user->can('order.invoice.view')) {
            return redirect()->route('admin.index')->with('failed', 'You don\'t have enough privileges to perform this action!');
        }
        ...
        return Datatables::of($data)->addIndexColumn()
            ...
            ->addColumn('other_info', function ($row) {
                $html = '';
                $html .= '<p><b>Invoice No: </b>'.'MS' . date('Y', strtotime($row->created_at)) . $row->id.'</p>';
                return $html;
            })
            ...
            ->addColumn('action', function ($row) {
                ...
                $btn = '<a class="btn btn-sm btn-success" target="_blank" href="'.route('admin.order.sold.invoice.print', ['pos', $row->id]).'">POS</a>';
                
                
                $btn = $btn . '<a class="btn btn-sm btn-info" target="_blank" href="'.route('admin.order.sold.invoice.print', ['invoice', $row->id]).'">Invoice</a>';
                
                
                $btn = $btn . '<a class="btn btn-sm btn-danger" target="_blank" href="'.route('admin.order.sold.invoice.print', ['halfpage', $row->id]).'">Half Page</a>';
                

                return $btn;
            })

            ->rawColumns(['created_at', 'user_id',  'other_info', 'action'])->make(true);
    }

    public function printInvoice($type, $id){
        $order = Order::find($id);
        if ($type == 'pos') {
            return view('backend.pages.order.invoice.pos', compact('order'));
        }elseif ($type == 'invoice') {
