...
class ProductsController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        //     for ($x = 0; $x < count($option); $x++) {
        //         $option[$x]['value'] = array_values($option[$x]['value']);
        //         for($y = 0; $y < count($option[$x]['value']); $y++){
        //             $option[$x]['value'][$y]['price'] = 20;
        //         }
        //     }
        ...
        $product->max_cart_qty = $request->max_cart_qty ?? 5;
        $product->minimum_cart_value = $request->minimum_cart_value ?? 0;
        $product->in_stock = $request->in_stock ?? $product->in_stock;
        $product->viewed = 1;
        $product->is_active = $request->is_active ? 1 : 0;
        $product->is_promotion = $request->is_promotion ? 1 : 0;
    ...
    public function getProductList()
    {
        ...
        return Datatables::of($data)
            ...
            ->rawColumns(['checkbox', 'title', 'sku', 'category_id', 'seller_id', 'product_type', 'price', 'weight', 'status', 'action'])->make(true);
    }
    ...
    public function changePaymentStatus(Request $request)
    {
        ...
        if ($transaction_id &&  $status_id) {
            ...
            if ($order) {
                $total_order_items = OrderDetails::where('order_id', $order->id)->count();
                ...
                if ($request->status_id == 2 || $request->status_id == 6) {
                    ...
                    if($request->status_id == 2){
                        ...
                        foreach($order_details as $details){
                            ...
                            if($product->product_type == 'simple' || $product->product_type == 'variable'){
                                if ($details->seller_id) {
                                    $shop = ShopInfo::select('name', 'slug')->where('seller_id', $details->seller_id)->first();
                                    $details->shop_name = $shop->name;
                                    $details->shop_slug = $shop->slug;
                                }
        
                                $SellerWiseGroup[$details->seller_id]['shop_info'] = [
                                    'shop_name' => $details->shop_name,
                                    'shop_slug' => $details->shop_slug,
                                    'seller_id' => $details->seller_id,
                                ];
                
                                $SellerWiseGroup[$details->seller_id]['items'][] = $details;
                            }
                        }
                        ...
                        foreach($SellerWiseGroup as $group){
                            ...
                            $merchant_order_id = 'MB'. date('y', strtotime($order->created_at)) .$details->order_id;
                            $recipient_name = $address->shipping_first_name.' '.$address->shipping_last_name;
                            $recipient_phone = $address->shipping_phone;
                            $recipient_address = $address->shipping_address; 
                            $recipient_city = $address->district->city_id;
                            $recipient_zone = $address->upazila->zone_id;
                            $recipient_area = $address->union->area_id;
                            $item_weight = 0;
                            $amount_to_collect = 0;
                            $productdetails = '';
                            $count = 1;
                            foreach($group['items'] as $details){
                                ...
                                $item_quantity = $details->product_qty;
                                if(Helper::convertProductWeightToKg($details->product_id) >= 0.1){
                                    $item_weight = $item_weight + (Helper::convertProductWeightToKg($details->product_id) * $details->product_qty);
                                }else{
                                    $item_weight = $item_weight + 0.1;
                                }
                                if($order->payment_method == 'cash_on_delivery'){
                                    $amount_to_collect = $order->total_amount - $order->paid_amount;
                                }else{
                                    $amount_to_collect = $amount_to_collect + 0;
                                }
                                
                                $productdetails = $productdetails.' '.$count.'. Product: '.$product->title.'</br> Price: '.$details->price.'</br>';
                            ...
                            $response = Helper::pathaoMakeOrder($store_id, $merchant_order_id, $recipient_name, $recipient_phone, $recipient_address, $recipient_city, $recipient_zone, $recipient_area, $delivery_type, $item_type, $special_instruction, $item_quantity, $item_weight, $amount_to_collect, $item_description);
