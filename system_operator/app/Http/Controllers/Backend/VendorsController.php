...
class VendorsController extends Controller
{
  ...
  public function getSellerList()
  {
    ...
    return Datatables::of($data)->addIndexColumn()
      ...
      ->addColumn('is_branch', function ($row) {
        if($row->is_branch == 1){
          return'<label class="badge badge-success">Branch</label>';
        }else{
          return'<label class="badge badge-info">Seller</label>';
        }
      })
      ...
      ->addColumn('action', function ($row) {
        ...
        if ($row->shopinfo->store_id == NULL) {
          $btn = $btn . '<a class="icon_btn text-info" title="Create Pathao Store" href="'.route('admin.vendor.create.pathao.store', $row->id).'" ><i class="mdi mdi-library-plus"></i></a>';
        }
      ...
      ->rawColumns(['checkbox','is_branch', 'seller', 'role', 'status', 'shop_status', 'action'])->make(true);
  }
  ...
  public function view($id)
  {
    ...
    $is_branch = ($vendor->is_branch == 1) ? 'Yes' : 'No';
  ...
  public function store(Request $request)
  {
    ...
    $vendor->is_branch = $request->is_branch ? 1 : 0;
    $vendor->is_branch = $request->is_branch ? 1 : 0;
  ...
  public function update(Request $request, $id)
  {
    ...
    $vendor->nid_front_side = $request->nid_front_side;
    $vendor->nid_back_side = $request->nid_back_side;
    $vendor->pathao_store_id = $request->pathao_store_id;
  ...
  public function createPathaoStore($vendor_id){

    $vendor = Admins::find($vendor_id);
    $shopInfo = ShopInfo::where('seller_id', $vendor_id)->first();

    //create pathao store
    $district = District::find($shopInfo->district);
    $upazila = Upazila::find($shopInfo->area);
    $union = Union::find($shopInfo->shop_union);

    $pathao_store = Helper::createPathaoStore($shopInfo->name, $vendor->name, $shopInfo->phone, $shopInfo->address, $vendor->phone, $district->city_id, $upazila->zone_id, $union->area_id);

    // var_dump($pathao_store->errors->address);
    // exit;

    if($pathao_store->type == 'success'){
      $update_shopinfo = ShopInfo::find($shopInfo->id);
      $update_shopinfo->store_id = $pathao_store->data->store_id;
      $update_shopinfo->save();
      return redirect()->route('admin.vendor')->with('success', $pathao_store->message);
    }else{
      return redirect()->route('admin.vendor')->with('failed', $pathao_store->message);
    }
