...
class ApiController extends Controller
{
	...
	public function site_info(){
		...
		if (\Helper::getsettings('default_shipping_enable') == 1){
			$data['default_shipping_enable'] = '1';
		}else{
			$data['default_shipping_enable'] = '0';
		}
	...
	public function sellerInformation(Request $request)
	{
		if ($request->seller_id) {
			...
			$reviewData = [];
			$reviews = DB::table('comments')->whereIn('commentable_id', $product_ids)->groupBy('id')->get();
			$reveiwed_products = [];
			$average = 0;
			if (count($reviews) > 0) {
				...
				$reviewData['five_star']  = count(DB::table('comments')->whereIn('commentable_id', $product_ids)->where('rate', 5)->get());
				$reviewData['five_star_percentage'] = 'width:' . number_format($reviewData['five_star'] / $reviewData['total_ratings'] * 100, 0, '.', '') . '%;';
				$reviewData['four_star']  = count(DB::table('comments')->whereIn('commentable_id', $product_ids)->where('rate', 4)->get());
				$reviewData['four_star_percentage'] = 'width:' . number_format($reviewData['four_star'] / $reviewData['total_ratings'] * 100, 0, '.', '') . '%;';
				$reviewData['three_star']  = count(DB::table('comments')->whereIn('commentable_id', $product_ids)->where('rate', 3)->get());
				$reviewData['three_star_percentage'] = 'width:' . number_format($reviewData['three_star'] / $reviewData['total_ratings'] * 100, 0, '.', '') . '%;';
				$reviewData['two_star']  = count(DB::table('comments')->whereIn('commentable_id', $product_ids)->where('rate', 2)->get());
				$reviewData['two_star_percentage'] = 'width:' . number_format($reviewData['two_star'] / $reviewData['total_ratings'] * 100, 0, '.', '') . '%;';
				$reviewData['one_star']  = count(DB::table('comments')->whereIn('commentable_id', $product_ids)->where('rate', 1)->get());
	...
	public function getDailyGroceryDealsProduct()
	{
	...
	{
		$data = Product::where('is_active', 1)->where('is_deleted', 0)->where('is_promotion', 0)->orderby('shuffle_number', 'desc')->where('product_qc', 1)->limit(18)->get();
		return response()->json($data, 200);
	}
   ...
   //  			}
	...
	public function initWishlist(Request $request)
	{
		...
		if ($user || $session_key) {
			...
			$data['total'] = count($wishlistData) ?? 0;
			return response()->json($data, 200);
		} else {
			$data['status'] = 0;
			$data['message'] = 'You have to login first to avail this option.';
	...
	public function AllRating($productId)
	{
		...
		if ($product) {
			...
			$data['total_rating'] = $product->totalCommentsCount() > 0 ? $product->totalCommentsCount() : 1;
			$data['total_rate'] = $product->totalCommentsCount() > 0 ? $product->totalCommentsCount() : 0;
		} else {
			$data['average'] =  0;
			$data['total_rating'] = 1;
			$data['total_rate'] = 0;
		...
		$data['five_star']  = count(DB::table('comments')->where('commentable_id', $productId)->where('rate', 5)->get());
		$data['five_star_percentage'] = 'width:' . number_format($data['five_star'] / $data['total_rating'] * 100, 0, '.', '') . '%;';
		$data['four_star']  = count(DB::table('comments')->where('commentable_id', $productId)->where('rate', 4)->get());
		$data['four_star_percentage'] = 'width:' . number_format($data['four_star'] / $data['total_rating'] * 100, 0, '.', '') . '%;';
		$data['three_star']  = count(DB::table('comments')->where('commentable_id', $productId)->where('rate', 3)->get());
		$data['three_star_percentage'] = 'width:' . number_format($data['three_star'] / $data['total_rating'] * 100, 0, '.', '') . '%;';
		$data['two_star']  = count(DB::table('comments')->where('commentable_id', $productId)->where('rate', 2)->get());
		$data['two_star_percentage'] = 'width:' . number_format($data['two_star'] / $data['total_rating'] * 100, 0, '.', '') . '%;';
		$data['one_star']  = count(DB::table('comments')->where('commentable_id', $productId)->where('rate', 1)->get());
	...
	public function sellerProductsBySlug($seller_slug){
		...
		if ($seller->seller_id) {
			...
			$data['ratings'] = $reviewData;
			$data['product'] = $seller_products;
			$data['total_products'] = count($seller_products);
			return response()->json($data, 200);
		} else {
			$data['status'] = 1;
			$data['product'] = null;
	...
	public function checkReviewed(Request $request)
	{
		if ($request->product_id && $request->user_id) {
			$already = DB::table('comments')->where('commentable_id', $request->product_id)->where('commented_id', $request->user_id)->first();
			if ($already) {
	...
	public function getSimillarProduct(Request $request)
	{
		...
		$simillarProducts = Product::where('category_id', $categoryId)->where('id', '!=', $produt_id)
			->where('is_active', 1)
			->where('is_deleted', 0)
			->where('product_qc', 1)
			->where('product_qc', 1)
	...
	public function initCart(Request $request)
	{
		...
		if ($user || $session_key) {
			...
			foreach ($cartData as $single_item) {
				...
				$total_weight_in_kg += (\Helper::convertProductWeightToKg($singleProduct->id) * $single_item->qty);

				if ($single_item->product_type != 'variable' || $single_item->product_type == 'digital') {
					$single_item->total_weight = \Helper::convertProductWeightToKg($singleProduct->id) * $single_item->qty;
					$single_item->weight_unit = $singleProduct->weight_unit;
				}
			...
			if ($user) {
				...
				if ($user->default_pickpoint_address == 1) {
					...
					if (\Helper::getsettings('default_shipping_enable') == 1){
						$default_address = Addresses::find($user->default_address_id);

						if ($default_address && in_array($default_address->shipping_district, $default_shipping_inside_location_ids)){
							$default_shipping_inside_origin = \Helper::getSettings('default_shipping_inside_origin');
							if($total_weight_in_kg > 1){
								$default_shipping_increase_per_kg = \Helper::getSettings('default_shipping_increase_per_kg');

								$shipping_cost += ($default_shipping_inside_origin + ($total_weight_in_kg * $default_shipping_increase_per_kg));
							}else{
								$shipping_cost += $default_shipping_inside_origin;
							}
							
						}else{
							$default_shipping_inside_origin = \Helper::getSettings('default_shipping_outside_origin');
							if($total_weight_in_kg > 1){
								$default_shipping_increase_per_kg = \Helper::getSettings('default_shipping_increase_per_kg');
					...
					$data['total_weight_in_kg'] = $default_shipping_inside_origin;
	...
	public function getPromotionProduct($product_type)
	{
	...
	public function getSellerReview()
	{
		...
		foreach ($vendorArray as $row) {
			...
			foreach (Product::where('seller_id', $row->id)->where('is_active', 1)->where('is_deleted', 0)->where('product_qc', 1)->orderby('shuffle_number', 'desc')->get() as $product) {
				$total_rating += DB::table('comments')->where('commentable_id', $product->id)->sum('rate');
				$rating_given_by += count(DB::table('comments')->where('commentable_id', $product->id)->whereNotNull('rate')->get());
			}
	...
	public function groceryProductsYouMayLike()
	{
		$products =  Product::where('is_active', 1)
			->where('is_deleted', 0)
			->where('product_qc', 1)
