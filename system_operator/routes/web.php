...
Route::group(['prefix' => 'admin', 'middleware' => 'auth'], function () {
    ...
    Route::group(['prefix' => '/pos'], function () {
        ...
        Route::post('/apply/discount', 'Backend\PosController@applyDiscount');

        Route::post('/change/cart/price', 'Backend\PosController@changeCartProductPrice');
    ...
    Route::group(['prefix' => '/orders'], function () {
        ...
        Route::get('/sold-invoice', 'Backend\OrdersController@soldInvoices')->name('admin.order.sold.invoice');
        Route::get('/get/sold-invoice', 'Backend\OrdersController@getSoldInvoices')->name('admin.order.get.sold.invoice');

        Route::get('/print-invoice/{type}/{invoice_number}', 'Backend\OrdersController@printInvoice')->name('admin.order.sold.invoice.print');
    });
    ...
    Route::group(['prefix' => '/seller'], function () {
        ...
        Route::get('/create/pathao/store/{id}', 'Backend\VendorsController@createPathaoStore')->name('admin.vendor.create.pathao.store');
    });
    ...
    Route::group(['prefix' => '/accounts'], function () {
        Route::get('/', 'Backend\AccountsController@dashboard')->name('admin.accounts.dashboard');
        Route::get('/cashflow', 'Backend\AccountsController@cashflow')->name('admin.accounts.cashflow');
        Route::get('/account-history', 'Backend\AccountsController@accountHistry')->name('admin.accounts.history');

        // customer due recieved 
        Route::get('/customer-due', 'Backend\AccountsController@customerDue')->name('admin.accounts.customer.due');
        Route::get('/customer-due/data', 'Backend\AccountsController@customerDuePaymentData')->name('admin.accounts.customer.due.data');
        Route::post('/customer-due/store', 'Backend\AccountsController@customerDueStore')->name('admin.accounts.customer.due.store');
        Route::get('/customer/history/{id}', 'Backend\AccountsController@getCustomerHistory')->name('admin.accounts.customer.history');

        // Supplier pay  
        Route::get('/supplier-pay', 'Backend\AccountsController@supplierDue')->name('admin.accounts.supplier.due');
        Route::get('/supplier-due/data', 'Backend\AccountsController@supplierDuePaymentData')->name('admin.accounts.supplier.due.data');
        Route::post('/supplier-due/store', 'Backend\AccountsController@supplierDueStore')->name('admin.accounts.supplier.due.store');
        Route::get('/supplier/history/{id}', 'Backend\AccountsController@getSupplierHistory')->name('admin.accounts.supplier.history');

        // Balance Transfer   
        Route::get('/balance-transfer', 'Backend\AccountsController@balanceTransfer')->name('admin.accounts.balance.transfer');
        Route::get('/balance-transfer/data', 'Backend\AccountsController@balanceTransferData')->name('admin.accounts.balance.transfer.data');
        Route::post('/balance-transfer/store', 'Backend\AccountsController@balanceTransferStore')->name('admin.accounts.balance.transfer.store');
        Route::get('/balance-transfer/history/{id}', 'Backend\AccountsController@getBalanceTransferHistory')->name('admin.accounts.balance.transfer.history');

        Route::get('/accounts', 'Backend\AssetController@index')->name('admin.accounts.assets');
        Route::get('/accounts/data', 'Backend\AssetController@getAssetData')->name('admin.accounts.assets.data');
        Route::post('/accounts/store', 'Backend\AssetController@store')->name('admin.accounts.asset.store');
        Route::post('/accounts/update', 'Backend\AssetController@update')->name('admin.accounts.asset.update');

        Route::get('/investor', 'Backend\InvestmentController@index')->name('admin.accounts.investor');
        Route::get('/investor/data', 'Backend\InvestmentController@getInvestorData')->name('admin.accounts.investor.data');
        Route::post('/investor/store', 'Backend\InvestmentController@store')->name('admin.accounts.investor.store');
        Route::post('/investor/update', 'Backend\InvestmentController@update')->name('admin.accounts.investor.update');
        Route::get('/investor/payment/data/{id}', 'Backend\InvestmentController@getInvestorPaymentData')->name('admin.accounts.investor.payment.data');


        Route::get('/loans', 'Backend\LoanController@index')->name('admin.accounts.loans');
        Route::get('/loans/data', 'Backend\LoanController@getLoanData')->name('admin.accounts.loans.data');
        Route::post('/loans/store', 'Backend\LoanController@store')->name('admin.accounts.loans.store');
        Route::post('/loans/update', 'Backend\LoanController@update')->name('admin.accounts.loans.update');
        Route::post('/loan-investor-payment', 'Backend\LoanController@loanAndInvestorPayment')->name('admin.accounts.loan.investor.payment');
        Route::get('/loans/payment/data/{id}', 'Backend\LoanController@getLoanPaymentData')->name('admin.accounts.loans.payment.data');
        // Route::post('/loan', 'Backend\AccountsController@loanAdd')->name('admin.accounts.loan.store');
        // Route::post('/loan-investor-payment', 'Backend\AccountsController@loanAndInvestorPayment')->name('admin.accounts.loan.investor.payment');
    ...
    Route::group(['prefix' => '/location'], function () {
        ...
        Route::group(['prefix' => '/district'], function () {
            ...
            Route::get('/generate', 'Backend\DistrictController@generate')->name('admin.location.district.generate');
            Route::get('/generate/single/{id}', 'Backend\DistrictController@generateSingle')->name('admin.location.district.generate.single');
        });
    });
    ...
    Route::group(['prefix' => '/location'], function () {
        ...
        Route::group(['prefix' => '/upazila'], function () {
            ...
            Route::get('/generate', 'Backend\UpazilaController@generate')->name('admin.location.upazila.generate');
            Route::get('/generate/single/{upazila_id}', 'Backend\UpazilaController@generateSingleArea')->name('admin.location.upazila.generate.single');
        });
    });
    ...
    Route::group(['prefix' => '/location'], function () {
        ...
        Route::group(['prefix' => '/union'], function () {
            ...
            Route::get('/generate', 'Backend\UnionController@generate')->name('admin.location.union.generate');
        });
    });
    ...
    Route::group(['prefix' => '/expense'], function () {
        Route::get('/list', 'Backend\ExpenseController@expenseList')->name('admin.expense');
        Route::get('/get-expense-list', 'Backend\ExpenseController@getExpenseList')->name('admin.get.expense.list');
        Route::get('/create', 'Backend\ExpenseController@expenseCreate')->name('admin.expense.create');
        Route::post('/store', 'Backend\ExpenseController@expenseStore')->name('admin.expense.store');
        Route::get('/edit/{id}', 'Backend\ExpenseController@expenseEdit')->name('admin.expense.edit');
        Route::post('/update', 'Backend\ExpenseController@expenseUpdate')->name('admin.expense.update');
        Route::get('/delete/{id}', 'Backend\ExpenseController@expenseDelete')->name('admin.expense.delete');

        Route::get('/report', 'Backend\ExpenseController@expenseReport')->name('admin.expense.report');
        Route::get('/get-expense-report', 'Backend\ExpenseController@getExpenseReport')->name('admin.get.expense.report');

        Route::post('/category/create', 'Backend\ExpenseController@expenseCategoryCreate')->name('admin.expense.category.create');
        Route::get('/category/delete/{id}', 'Backend\ExpenseController@expenseCategoryDelete')->name('admin.expense.category.delete');
    });
    ...
    //supplier Routes for admin
    Route::group(['prefix' => '/supplier'], function () {
        Route::get('/', 'Backend\SupplierController@index')->name('admin.supplier');
        Route::get('/get-supplier-list', 'Backend\SupplierController@getSupplierList')->name('admin.get.supplier.list');

        Route::get('/create', 'Backend\SupplierController@create')->name('admin.supplier.create');
        Route::get('/edit/{id}', 'Backend\SupplierController@edit')->name('admin.supplier.edit');
        Route::post('/store', 'Backend\SupplierController@store')->name('admin.supplier.store');
        Route::post('/update/{id}', 'Backend\SupplierController@update')->name('admin.supplier.update');
        Route::get('/delete/{id}', 'Backend\SupplierController@delete')->name('admin.supplier.delete');

        Route::post('/bulk/action', 'Backend\SupplierController@action')->name('admin.supplier.bulk.action');
        Route::get('/ledger/{id}', 'Backend\SupplierController@view')->name('admin.supplier.view');
        Route::get('/product/ledger/{id}', 'Backend\SupplierController@viewProducts')->name('admin.supplier.product.ledger');

    });

    Route::group(['prefix' => '/purchase'], function () {
        Route::get('/', 'Backend\PurchaseController@index')->name('admin.purchase');
        Route::get('/get-purchase-list/{supplier_id}', 'Backend\PurchaseController@getPurchaseList')->name('admin.get.purchase.list');

        Route::get('/get-payment-list/{supplier_id}', 'Backend\PurchaseController@getPaymentList')->name('admin.get.payment.list');

        Route::get('/product/details/{id}', 'Backend\PurchaseController@productDetails')->name('admin.purchase.product.details');

        Route::get('/create', 'Backend\PurchaseController@create')->name('admin.purchase.create');
        Route::get('/edit/{id}', 'Backend\PurchaseController@edit')->name('admin.purchase.edit');
        Route::post('/store', 'Backend\PurchaseController@store')->name('admin.purchase.store');
        Route::post('/update/{id}', 'Backend\PurchaseController@update')->name('admin.purchase.update');
        Route::get('/delete/{id}', 'Backend\PurchaseController@delete')->name('admin.purchase.delete');

        Route::post('/bulk/action', 'Backend\PurchaseController@action')->name('admin.purchase.bulk.action');
        Route::get('/view/{id}', 'Backend\PurchaseController@view')->name('admin.purchase.view');
        Route::post('/due/payment', 'Backend\PurchaseController@duePayment')->name('admin.purchase.due.payment');

       

        // return 
        Route::get('/get-purchase-return-list/{supplier_id}', 'Backend\PurchaseController@getPurchaseReturnList')->name('admin.get.purchase.return.list');

        Route::get('/return', 'Backend\PurchaseController@return')->name('admin.purchase.return');
        Route::get('/return/create', 'Backend\PurchaseController@returncreate')->name('admin.purchase.return.create');
        Route::get('/return/edit/{id}', 'Backend\PurchaseController@returnedit')->name('admin.purchase.returnedit');
        Route::post('/return/store', 'Backend\PurchaseController@returnstore')->name('admin.purchase.return.store');
        Route::get('/return/confirm/{id}', 'Backend\PurchaseController@returnConfirm')->name('admin.purchase.return.confirm');
        Route::get('/return/delete/{id}', 'Backend\PurchaseController@returndelete')->name('admin.purchase.return.delete');
        Route::get('/return/view/{id}', 'Backend\PurchaseController@returnview')->name('admin.purchase.return.view');
        Route::get('/get/supper/invoice', 'Backend\PurchaseController@getInvoices');
        Route::get('/invoice/products', 'Backend\PurchaseController@getInvoicesProducts');
        Route::get('/invoice/products/details', 'Backend\PurchaseController@getInvoicesProductsDetails');
    });
